<div class="archiveposts">
			...
			<?php if ($wpzoom_homepost_thumb  == 'Show') { ?>	
				<?php if ( current_theme_supports( 'post-thumbnails' ) && has_post_thumbnail() ) {
					...
					if ($img){ 
						$img = wpzoom_wpmu($img);
					?>
				<div class="cover_meme"><a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title(); ?>"><img src="<?php bloginfo('template_directory'); ?>/scripts/timthumb.php?src=<?php echo $img ?>&amp;w=<?php echo "$wpzoom_homepost_thumb_width";?>&amp;h=<?php echo "$wpzoom_homepost_thumb_height";?>&amp;zc=1" alt="<?php the_title(); ?>" /></a></div>
				<?php } ?>
			<?php } ?>
	...
	<div class="cleaner">&nbsp;</div>
		...
		<div class="navigation">
			<?php if ( function_exists( 'page_navi' ) ) page_navi( 'items=10&prev_label=ÇáÓÇÈÞ&next_label=ÇáÊÇáí&first_label=ÇáÃæáì&last_label=ÇáÃÎíÑÉ&num_position=after' ); ?>
		</div> 
		<?php else : ?>
